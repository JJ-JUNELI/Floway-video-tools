...
### 第 0 步（前置）：给 4 个图表加控件默认值快照
- 现在只有 card-3d 有 `tests/snapshots/card-3d.json`。先给 chart-fx/multi-line/bar-chart/pie-chart 各生成一份快照（沿用 defaults.spec.js 机制）。
- **作用**：这是整个重构的安全网——之后每一步都跑快照，逐字节不变才算"只去重没改值"。
- 风险：极低（纯加测试）。
