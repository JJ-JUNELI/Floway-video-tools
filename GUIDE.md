...
为让用户在不同效果间切换时「肌肉记忆」不失效，所有效果面板遵循以下约定。新增/改动效果时务必对齐。

**1. 控件布局方向**
- **滑块（range）→ 竖排 `.stack`**：标签行在上、滑条在下（单个用 `.row.stack`，两个并排用 `.row` 内放两个 `.stack flex:1`）。
- **下拉 / 颜色 / 开关 → 横排 `.row`**：横排更紧凑。
- 不要把滑块塞进横排、也不要把下拉/颜色拆成竖排——方向跟控件类型走，不跟效果走。

**2. 数值单位 —— 默认不加（保持简洁，避免视觉杂乱）**
- 滑块显示值**默认是裸数字**（如 `150`、`48`），**不要**给字号/像素/距离/点大小等加 `px` 等后缀——加了视觉杂乱，是刻意去掉的。
- 仅保留少数**起消歧作用**的既有单位：透明度 `%`（值域 `min=0 max=100`）、角度 `°`、时长 `s`。这些已在用、不要去掉，也不要扩展到其它量。
- 透明度内部若是 0–1，用 `bindUI` 的 `'%'` transform（值×100 显示、÷100 存储）。除此之外不要为了"统一"去批量加单位。

**3. 分组标题**
- **统一中文**，去中英混用：`核心素材 (Logo)` → `核心素材`、`粒子参数` 保持。
- 标题里的纯文本标签**必须包 `<span>`**：`<div class="group-title"><span>颜色设置</span></div>`。否则裸文本节点匹配不到 `.group-title > span:first-child` 的 `flex:1`，折叠时「还原 ↺ / 折叠 ▼」按钮不会归位到行尾（本会话踩过）。

**4. 分组顺序（从上到下）**
```
模式/内容  →  样式  →  排版与位置  →  动画  →  （背景 + 导出，由 initEffect 自动注入在末尾）
```

**5. checkbox 语义区分**
- **二元功能开关**（开/关）→ toggle 开关（`.sub-title` + checkbox），如「显示图例」「启用辉光」。
- **三选一及以上的模式/枚举**（互斥）→ 用 `<select>` 或分段控件（`.cat-tabs` 同款），**不要**用一排并列 checkbox 表达互斥选择（chart-fx 曾 14 个一样的 toggle 难分开关 vs 模式）。

**6. ID 命名（已落地，保持）**
- 控件 `Xxx`、显示值 `XxxVal`（如 `Angle` / `AngleVal`）。
- 例外：`MainFontInput` / `BgFontInput` 是 `setupFontSelector` 的契约参数，保留。

> ⚠️ **改控件顺序会破快照测试**：`tests/snapshots/card-3d.json` 按 DOM 顺序 dump 所有 `input[id]/select[id]/textarea[id]` 的值。调整 card-3d 的控件顺序后，必须同步更新该快照（`npm test` 会红）。
